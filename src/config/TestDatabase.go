...
func (t TestDatabase) Connect() (*gorm.DB, error) {
	db, err := gorm.Open(sqlite.Open(t.DSN), &gorm.Config{Logger: logger.Default.LogMode(logger.Silent)})
	if err != nil {
...
func (TestDatabase) Disconnect(db *gorm.DB) {
	database, _ := db.DB()
	database.Close()
}
